...
const { createContactUs, findContactUs, updateContactUs, contactUsList, updateContactUsById } = contactUsServices;
import { feedbackServices } from '../../services/feedback'
const { createFeedback, findFeedback, updateFeedback, updateFeedbackById, feedbackList } = feedbackServices
    ...
    */
    async viewContactUs(req, res, next) {
        ...
        try {
            ...
            let result = await findContactUs({ _id: _id, status: { $ne: status.DELETE } });
    ...
    async replyContactUs(req, res, next) {
        ...
        try {
            ...
            let contactUsResult = await findContactUs({ _id: validatedBody._id, status: { $ne: status.DELETE } });
            if (!contactUsResult) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND);
            }
            let result = await updateContactUsById(contactUsResult._id, { $set: { replyMessage: validatedBody.message } });
    ...
    async deleteContactUs(req, res, next) {
        ...
        try {
            ...
            let contactUsResult = await findContactUs({ _id: _id, status: { $ne: status.DELETE } });
            if (!contactUsResult) {
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND);
            }
            let result = await updateContactUsById(_id, { $set: { status: status.DELETE } });
    ...
    */
    async viewFeedback(req, res, next) {
        ...
        try {
            ...
            let saveQuery = await findFeedback({_id:validatedBody.feedbackId,status: { $ne: status.DELETE }});
            if(!saveQuery){
                throw apiError.notFound(responseMessage.DATA_NOT_FOUND);
            }
            return res.json(new response(saveQuery, responseMessage.ADD_FEEDBACK));
